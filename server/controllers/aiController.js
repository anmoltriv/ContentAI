...
import { removeBackgroundFromImage, removeObjectFromImage } from "../utils/modifyImage.js";
...
export const removeObject = async (req, res) => {
    try {
        ...
        if (!prompt) {
            return res.status(400).json({ success: false, message: "A description prompt of the object to remove is required." });
        }
        ...
        const result = await removeObjectFromImage(req.file, prompt);
        ...
        console.error("Error in removeObject controller:", error.message);
        return res.status(500).json({ success: false, message: error.message });
    }
};
